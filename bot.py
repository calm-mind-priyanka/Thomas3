...
# Admin commands - Bot 1 (PM only)
@client1.on(events.NewMessage)
async def bot1_commands(e):
    global msg1, delay1, gap1
    if e.sender_id != ADMIN1 or e.is_group: return
    txt = e.raw_text.strip()
    if txt.startswith("/addgroup"):
        try:
            group_id = int(txt.split(" ", 1)[1])
            groups1.add(group_id); save_groups(GROUPS_FILE1, groups1)
            await e.reply(f"✅ Group `{group_id}` added.")
        except: await e.reply("❌ Usage: /addgroup -100xxxx")
    elif txt.startswith("/removegroup"):
        try:
            group_id = int(txt.split(" ", 1)[1])
            groups1.discard(group_id); save_groups(GROUPS_FILE1, groups1)
            await e.reply(f"❌ Group `{group_id}` removed.")
        except: await e.reply("❌ Usage: /removegroup -100xxxx")
...
# Admin commands - Bot 2 (PM only)
@client2.on(events.NewMessage)
async def bot2_commands(e):
    global msg2, delay2, gap2
    if e.sender_id != ADMIN2 or e.is_group: return
    txt = e.raw_text.strip()
    if txt.startswith("/addgroup"):
        try:
            group_id = int(txt.split(" ", 1)[1])
